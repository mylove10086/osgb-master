...
public:
    ...
    Canvas() {
        ...
        fclose(file);
        std::vector<float> *vv = (std::vector<float> *) &(osgb.pagedLod[0]->geode[0]->geometry->vertex);
    ...
    void init() {
        ...
        int siezv = osgb.pagedLod[0]->geode[0]->geometry->vertex.size();

        int size = 4 * vertices->size();
        glBufferData(bufferTarget, siezv, &(osgb.pagedLod[0]->geode[0]->geometry->vertex[0]), usage);
        ...
        int siezi = osgb.pagedLod[0]->geode[0]->geometry->vertexIndex.size();
        glBufferData(bufferTargetindex, siezi, &(osgb.pagedLod[0]->geode[0]->geometry->vertexIndex[0]), usageindex);
    ...
    void setUnform() {
        ...
        glm::vec3 lightPos{813.897095, 503.618286, -35.6540108};
        glm::mat4 model = glm::mat4(1.0);
        model = glm::translate(model, lightPos);
        model = glm::scale(model, glm::vec3(1.00050,1,1.009));
        glUniformMatrix4fv(projection, 1, false, glm::value_ptr(model));//投影矩阵
...
int main() {
    ...
    model = glm::translate(model, lightPos);
    model = glm::scale(model, glm::vec3(1.00050,1,1.009));
    glm::dvec4 pos=model* glm::dvec4{-813.897095, -503.618286, 35.6540108,1};
